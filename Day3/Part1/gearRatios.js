...
const calculateSum = (text) => {
    ...
    for(let i = 0; i < allNumber.length ; i++ ) {
        for (let j = 0; j < allNumber[i].length; j++) {
            ...
            isNextElementNumber = j < allNumber[i].length - 1 && (allNumber[i][j+1].charCodeAt(0) < 48 || allNumber[i][j+1].charCodeAt(0) > 57)
            if(isPartNumber && (isNextElementNumber || j === allNumber[i].length - 1)) {
                sum = sum + parseInt(strNumber);
                strNumber = '';
            }
        }
    }
