const calculateCalibrationValues = (text) => {
    ...
    for(let i=0; i<textArr.length; i++) {
        ...
        let flag2 = false;
        while(j!==k) {
            const isFirstPointerAlphabet = textArr[i].charCodeAt(j) >= 97 && textArr[i].charCodeAt(j) <= 122;
            const isLastPointerAlphabet = textArr[i].charCodeAt(k) >= 97 && textArr[i].charCodeAt(k) <= 122;
            const isAlphabet = isFirstPointerAlphabet && isLastPointerAlphabet;
            if(isAlphabet) {
                number1 = number1 + textArr[i].charAt(j);
                number2 = textArr[i].charAt(k) + number2;
                j++;
                k--;
            } else if (isFirstPointerAlphabet) {
                number1 = number1 + textArr[i].charAt(j);
                j++;
            } else if (isLastPointerAlphabet) {
                number2 = textArr[i].charAt(k) + number2;
                k--;
            } else {
                break;
        ...
        if(resultString === '') {
            resultString = textArr[i].charAt(j).toString()+ textArr[i].charAt(k).toString();
        } else if (resultString.length === 1 && flag1) {
            resultString = resultString + textArr[i].charAt(k).toString();
        } else if (resultString.length === 1 && flag2) {
            resultString = textArr[i].charAt(j).toString() + resultString;
        }
        sum = sum + parseInt(resultString);
    }
    return sum;
}
